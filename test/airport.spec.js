...
describe('Test suite for airport.js', () => {
    ...
    it('should have military Planes with transport type', () => {
        ...
        let  hasTransportTypeForMilitaryPlanes = false;
        for (let militaryPlane of transportMilitaryPlanes) {
            if (militaryPlane.getMilitaryType() === MilitaryType.TYPE_TRANSPORT) {
                hasTransportTypeForMilitaryPlanes = true;
                break;
            }
        }
        assert.isTrue(hasTransportTypeForMilitaryPlanes);
    });
    ...
    it('should contain at least one bomber in military planes', () => {
        let airport = new Airport(planes);
        let bomberMilitaryPlanes  = airport.getBomberMilitaryPlanes();
        assert.isTrue(bomberMilitaryPlanes.length != 0);
    })

    it('should not contain Unclassified classification level in experimental planes', () => {
        let airport = new Airport(planes);
